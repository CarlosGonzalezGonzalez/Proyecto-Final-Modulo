	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if (id.equals("")) {
			...
			try{
				...
				salida.println(Configuration.getInstance().getProperty(Configuration.TITULO_ELIMINAR_PROFESOR) + 
	            		"/" + id);
				
				String[] datos = entrada.readLine().split("/");
				if(!datos[0].equals(" ")){
					salida.println("continuar");
					JOptionPane.showMessageDialog(frame, Configuration.getInstance().getProperty(Configuration.PROFESOR_ELIMINADO));
				}else{
